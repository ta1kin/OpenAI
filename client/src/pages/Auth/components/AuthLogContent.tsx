...
const AuthLogContent: React.FC<Props> = ( { isActive, 
                                            setActiveClass, 
                                            isLokType, 
                                            setLokClass,
                                            email, 
    ...
    return (
        <>
            <div className="form-box Login">
                    <h2 className={ `animation ${ isActive ? 'd-0' : 'l-10' }` }>Авторизация</h2>
                    <form>
                        <div className={ `input-box animation ${ isActive ? 'd-1' : 'l-10' }` }>
                            <input type="text" 
                                    id="email"
                                    value={email}
                        ...
                        <div className={ `input-box animation ${ isActive ? 'd-2' : 'l-12' }` }>
                        ...
                        <div className={ `reg-link animation ${ isActive ? 'd-3' : 'l-13' }` }>
                            <p >Нет аккаунта?</p>
                            <a href="#" className="SingUpLink" onClick={ setActiveClass }>Регистрация</a>
                        </div>
                        <div className={ `input-btn animation ${ isActive ? 'd-4' : 'l-14' }` }>
                            <button>Войти</button>
                        </div>
                    </form>
                    <div className={ `oauth-form animation ${ isActive ? 'd-5' : 'l-15' }` }>
                        <h3>Войти другим способом</h3>
                ...
                <div className="info-content Login">
                    <h2 className={ `animation ${ isActive ? 'd-0' : 'l-10' }` }>С ВОЗВРАЩЕНИЕМ!</h2>
                    <p className={ `animation ${ isActive ? 'd-1' : 'l-11' }` }>Lorem ipsum dolor sit amet consectetur, adipisicing elit. Dolor, vero?</p>
                </div>
        </>
    )
}

export default AuthLogContent